...
import { type MasterMenuItemType, type MasterMenuType, master_menu } from '@/config/menu.config'
...
export const Sidebar = () => {
  ...
  useEffect(() => {
    const findSubMenu = () => {
      ...
      let menu = master_menu.find((item: MasterMenuType) => item.href === pathname)

      if (!menu) {
        // サブメニュー項目の中で現在のパスに一致するものを探し、その親メニューを見つける
        for (const item of master_menu) {
  ...
  return (
    <aside className="sticky top-0 h-screen w-[250px] bg-turqb-500 text-white">
      <div className="flex flex-col self-stretch pb-4">
        ...
        {master_menu.map((item: MasterMenuItemType) => (
          <Link
            key={item.href}
            href={item.href}
        ...
        <div className="fixed bottom-1 px-4 py-2">
          <Link href="/admin/organizational_settings" passHref>
