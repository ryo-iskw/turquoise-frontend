...
const aggregatorTemplates = {
  ...
  sum(formatter = usFmt) {
    return ([attr]) => () => ({
          sum: 0,
          push(record) {
            if (!Number.isNaN(Number.parseFloat(record[attr]))) {
              this.sum += Number.parseFloat(record[attr]);
            }
          },
          value() {
            return this.sum;
  ...
  quantile(q, formatter = usFmt) {
    return ([attr]) => () => ({
          vals: [],
          push(record) {
            const x = Number.parseFloat(record[attr]);
            if (!Number.isNaN(x)) {
              this.vals.push(x);
            }
          },
  ...
  sumOverSum(formatter = usFmt) {
    return ([num, denom]) => () => ({
          ...
          sumDenom: 0,
          push(record) {
            if (!Number.isNaN(Number.parseFloat(record[num]))) {
              this.sumNum += Number.parseFloat(record[num]);
            }
            if (!Number.isNaN(Number.parseFloat(record[denom]))) {
              this.sumDenom += Number.parseFloat(record[denom]);
            }
          },
          value() {
...
PivotData.forEachRecord = (input, derivedAttributes, f) => {
  let addRecord;
  let record;
